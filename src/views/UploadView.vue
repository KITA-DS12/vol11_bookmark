  ...
  <v-app id="inspire">
    <v-main class="red lighten-4">
      <v-container style="padding-top: 10%;">
        <v-row>
          <v-col cols="12" sm="6">
            <v-sheet rounded="lg" min-height="100%" class="red lighten-4" style="text-align: center;">
              <input multiple type="file" ref="input" accept="html" style="display: none" @change="getFileContent">
              <svg style="fill: #555;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 20">
                <path
              ...
              <v-btn style="margin-top: 5%" width="70%" rounded class="white--text" color="teal lighten-1" x-large
                @click="inputButton">UPLOAD FILE</v-btn>
            </v-sheet>
          </v-col>
          <v-col cols="12" sm="6">
            <v-sheet min-height="70vh" rounded="lg" class="red lighten-4"
              style="text-align: center; position: relative;">
              <div style="height: 10vh" />
              <v-combobox v-model="targets" :items="folders" hide-selected multiple
                label="2. Select the target folder for classification." @input="onInputCombobox">
              ...
              <v-combobox v-model="chips" clearabkle hide-selected label="3. Please enter a name for the folder."
                multiple height="300">
              ...
              <v-btn v-bind:disabled="!isAllowedToPush" style="position: fixed; bottom: 10%; right: 5%;" rounded
                class="white--text" color="teal lighten-1" x-large @click="uploadFile">
                EXECUTE
                <v-icon right>
                  mdi-send
                </v-icon>
...
export default {
  ...
  methods: {
    ...
    async getFileContent() {
      try {
        ...
        for (const file of files) {
          ...
          this.contents.push(content.slice(22))
        }
    ...
    async readFolder() {
      await axios
        .post("html-json", {
          bookmark: this.contents,
        })
        .then((res) => {
          console.log(res.data)
          res.data.children.forEach((folder) => {
            this.removeDuplicates(folder.children)
            this.ids = []
          })
          this.response_json = res.data
          this.folders.push(res.data.title)
          this.exploreFolder(res.data.children)
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    exploreFolder(data) {
      data.forEach((child) => {
        if (child.type == "folder") {
          this.folders.push(child.title)
          this.exploreFolder(child.children)
        }
      })
    },
    removeDuplicates(data) {
      data.forEach((child, idx) => {
        ...
        if (this.type == "folder") {
          this.removeDuplicates(child.children)
        }
      })
    },
    async uploadFile() {
